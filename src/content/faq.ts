...
/**
 * NOT RENDERED -- awaiting Dr. Nupur's sign-off (audit item 40).
 *
 * These four were written by an earlier build pass (commit 9b98592), not
 * supplied by the clinic, and three of them give clinical advice or make a
 * claim about the practice (CLAUDE.md §1). The toothpaste answer in particular
 * gives "a pea-sized smear for toddlers", where standard pediatric guidance is
 * a rice-grain smear until age three and a pea-sized amount from three to six.
 * Move an item up into `FAQS` only once the clinic has approved its wording.
 */
export const FAQS_AWAITING_REVIEW: readonly FaqItem[] = [
  ...
  { question: 'What kind of toothpaste should we use at home?', answer: 'Use an age-appropriate fluoride toothpaste with a pea-sized smear for toddlers, or a tiny rice-grain dab for babies under two.', category: 'At home' },
